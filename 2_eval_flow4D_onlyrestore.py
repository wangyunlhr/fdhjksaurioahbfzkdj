...
from scripts.network.dataloader_flow4D import HDF5Dataset_multi_frame_idx_NK #!
from scripts.pl_model_flow4D_onlyrestore import ModelWrapper #!
...
def main(cfg):
    ...
    save_folder = parts[-1][:-5]
    output_dir = os.path.join(output_dir, save_folder)
    ...
    mymodel = ModelWrapper.load_from_checkpoint(cfg.checkpoint, cfg=cfg, eval=True, strict=False)
    # cfg = DictConfig(OmegaConf.to_container(cfg, resolve=True))
    # mymodel = ModelWrapper(cfg)

    wandb_logger = TensorBoardLogger(save_dir=output_dir, name="logs")
    trainer = pl.Trainer(logger=wandb_logger, log_every_n_steps=1, devices=1)

    trainer.validate(model = mymodel, \
                     dataloaders = DataLoader(HDF5Dataset_multi_frame_idx_NK(cfg.dataset_path + f"/{cfg.av2_mode}", n_frames=cfg.num_frames, eval=True), batch_size=1, shuffle=False, num_workers=num_workers))
    wandb.finish()
