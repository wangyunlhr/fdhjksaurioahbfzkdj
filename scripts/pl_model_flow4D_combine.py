...
from scripts.network.official_metric import PointMetrics, evaluate_leaderboard, evaluate_leaderboard_v2, OfficialMetrics
from assets.cuda.chamfer3D import nnChamferDis
MyCUDAChamferDis = nnChamferDis()
from scripts.network.models.combine_net import CombinedModel
...
class ModelWrapper(LightningModule):
    def __init__(self, cfg, eval=False):
        ...
        self.model1 = instantiate(cfg.model.target)
        self.model1.apply(weights_init)
        self.model2 = instantiate(cfg.model2.target)
        self.model2.apply(weights_init)
        self.model = CombinedModel(self.model1, self.model2)
        ...
        self.metrics_flow = OfficialMetrics()
    ...
    def training_step(self, batch, batch_idx):
        ...
        res_dict, flow_dict = self.model(batch, 'train')
        ...
        for batch_id in range(batch_sizes):
            ...
            total_loss_restore += chamferdis.mean()
        
        self.log("trainer/restore_loss", (total_loss_restore.item())/batch_sizes, sync_dist=True, batch_size=self.batch_size)


        gt_flow = batch['flow'] #gt_flow = ego+motion
        pose_flows = flow_dict['pose_flow'] #pose_flow = ego-motion's flow
        pc0_valid_idx = flow_dict['pc0_valid_point_idxes'] # since padding
        est_flow = flow_dict['flow'] #network's output, motion flow 
        ...
        for batch_id in range(batch_sizes):
            ...
            flow_loss_dict = {'est_flow': est_flow_, 
                        'gt_flow': gt_flow_, 
            ...
            flow_loss = self.loss_fn(flow_loss_dict)

            total_loss_flow += flow_loss
        
        self.log("trainer/flow_loss", (total_loss_flow.item())/batch_sizes, sync_dist=True, batch_size=self.batch_size)
        total_loss_all = total_loss_restore + total_loss_flow
        self.log("trainer/total_loss", (total_loss_all.item())/batch_sizes, sync_dist=True, batch_size=self.batch_size)
...
#! 修改成chamfer指标
    def train_validation_step_(self, batch, res_dict, flow_dict): 
        ...
        if batch['flow'][0].shape[0] > 0:
            pose_flows = flow_dict['pose_flow']
            for batch_id, gt_flow in enumerate(batch["flow"]):
                valid_from_pc2res = flow_dict['pc0_valid_point_idxes'][batch_id]
                pose_flow = pose_flows[batch_id][valid_from_pc2res]
                

                final_flow_ = pose_flow.clone() + flow_dict['flow'][batch_id]
                ...
                self.metrics_flow.step(v1_dict, v2_dict)
    ...
    def on_validation_epoch_end(self):
        ...
        self.metrics_flow.normalize()

        # wandb log things:
        for key in self.metrics_chamfer.point_bag:
            self.log(f"val/{key}", self.metrics_chamfer.point_bag[key], sync_dist=True)
        for key in self.metrics_flow.bucketed:
            for type_ in 'Static', 'Dynamic':
                self.log(f"val/{type_}/{key}", self.metrics_flow.bucketed[key][type_], sync_dist=True)
        for key in self.metrics_flow.epe_3way:
            self.log(f"val/{key}", self.metrics_flow.epe_3way[key], sync_dist=True)


        print("________chamfer________")
        self.metrics_chamfer.print()
        self.metrics_chamfer = PointMetrics()
        print("________flow________")
        self.metrics_flow.print()
        self.metrics_flow = OfficialMetrics()
    ...
    def validation_step(self, batch, batch_idx):
        if self.av2_mode == 'val' or self.av2_mode == 'test':
            ...
            res_dict, flow_dict = self.model(batch, 'val')
            self.train_validation_step_(batch, res_dict, flow_dict)
