...
class Seperate_to_pc0_pc1(nn.Module):
    def __init__(self):
        super(Seperate_to_pc0_pc1, self).__init__()
        # self.num_frames = num_frames
        #self.return_pc1 = return_pc1

    def forward(self, sparse_3D_tensor, batch_sizes):

        indices_3d = sparse_3D_tensor.indices
        features_3d = sparse_3D_tensor.features
        

        mask_pc0 = (indices_3d[:, 0] < batch_sizes)
        mask_pc1 = ~mask_pc0
        
        pc0_indices = indices_3d[mask_pc0]
        pc0_features = features_3d[mask_pc0]

        pc1_indices = indices_3d[mask_pc1]
        pc1_features = features_3d[mask_pc1]
        pc1_indices[:, 0] = pc1_indices[:, 0] - batch_sizes

        
        spatial_shape = sparse_3D_tensor.spatial_shape

        pc0_sparse_3D = spconv.SparseConvTensor(pc0_features.contiguous(), pc0_indices.contiguous(), \
                                                   spatial_shape, int(batch_sizes))
        pc1_sparse_3D = spconv.SparseConvTensor(pc1_features.contiguous(), pc1_indices.contiguous(), \
                                                   spatial_shape, int(batch_sizes))


        return pc0_sparse_3D, pc1_sparse_3D
...
class SpatioTemporal_Decomposition_3D(nn.Module):
    def __init__(self, in_filters, mid_filters, out_filters, indice_key=None, down_key = None, pooling=False, z_pooling=True, interact=False):
        super(SpatioTemporal_Decomposition_3D, self).__init__()
        ...
        self.spatial_conv_1 = conv3x3x3(in_filters, mid_filters, indice_key=indice_key + "bef")
        self.bn_s_1 = nn.BatchNorm1d(mid_filters)

        # self.temporal_conv_1 = conv1x1x1x3(in_filters, mid_filters)
        # self.bn_t_1 = nn.BatchNorm1d(mid_filters)

        self.fusion_conv_1 = conv1x1x1(mid_filters*1+in_filters, mid_filters, indice_key=indice_key + "1D")
        self.bn_fusion_1 = nn.BatchNorm1d(mid_filters)


        self.spatial_conv_2 = conv3x3x3(mid_filters, mid_filters, indice_key=indice_key + "bef")
        self.bn_s_2 = nn.BatchNorm1d(mid_filters)

        # self.temporal_conv_2 = conv1x1x1x3(mid_filters, mid_filters)
        # self.bn_t_2 = nn.BatchNorm1d(mid_filters)

        self.fusion_conv_2 = conv1x1x1(mid_filters*2, out_filters, indice_key=indice_key + "1D")
        self.bn_fusion_2 = nn.BatchNorm1d(out_filters)


        if self.pooling:
            if z_pooling == True:
                self.pool = spconv.SparseConv3d(out_filters, out_filters, kernel_size=(2,2,2), stride=(2,2,2), indice_key=down_key, bias=False)
            else:
                self.pool = spconv.SparseConv3d(out_filters, out_filters, kernel_size=(2,2,1), stride=(2,2,1), indice_key=down_key, bias=False)
    ...
    def forward(self, x):
        ...
        ST_feat_1 = x.replace_feature(torch.cat([S_feat_1.features, x.features], 1)) #residual concate
        ...
        ST_feat_2 = x.replace_feature(torch.cat([S_feat_2.features,  ST_feat_1.features], 1)) #residual concate
...
class Network_3D(nn.Module):
    def __init__(self, in_channel=16, out_channel=16, model_size = 16):
        ...
        SpatioTemporal_Block = SpatioTemporal_Decomposition_3D

        self.model_size = model_size
        

        self.STDB_1_1_1 = SpatioTemporal_Block(in_channel, model_size, model_size, indice_key="st1_13D", down_key='floor1_3D')
        self.STDB_1_1_2 = SpatioTemporal_Block(model_size, model_size, model_size*2, indice_key="st1_13D", down_key='floor1_3D', pooling=True) #512 512 32 -> 256 256 16

        self.STDB_2_1_1 = SpatioTemporal_Block(model_size*2, model_size*2, model_size*2, indice_key="st2_13D", down_key='floor2_3D')
        self.STDB_2_1_2 = SpatioTemporal_Block(model_size*2, model_size*2, model_size*4, indice_key="st2_13D", down_key='floor2_3D', pooling=True) #256 256 16 -> 128 128 8

        self.STDB_3_1_1 = SpatioTemporal_Block(model_size*4, model_size*4, model_size*4, indice_key="st3_13D", down_key='floor3_3D')
        self.STDB_3_1_2 = SpatioTemporal_Block(model_size*4, model_size*4, model_size*4, indice_key="st3_13D", down_key='floor3_3D', pooling=True) #128 128 8 -> 64 64 4

        self.STDB_4_1_1 = SpatioTemporal_Block(model_size*4, model_size*4, model_size*4, indice_key="st4_13D", down_key='floor4_3D')
        self.STDB_4_1_2 = SpatioTemporal_Block(model_size*4, model_size*4, model_size*4, indice_key="st4_13D", down_key='floor4_3D', pooling=True, z_pooling=False) #64 64 4 -> 64 64 4

        self.STDB_5_1_1 = SpatioTemporal_Block(model_size*4, model_size*4, model_size*4, indice_key="st5_13D")
        # self.STDB_5_1_2 = SpatioTemporal_Block(model_size*16, model_size*16, model_size*16, indice_key="st5_13D")
        self.up_subm_5 = spconv.SparseInverseConv3d(model_size*4, model_size*4, kernel_size=(2,2,1), indice_key='floor4_3D', bias=False) #zpooling false

        self.STDB_4_2_1 = SpatioTemporal_Block(model_size*8, model_size*8, model_size*4, indice_key="st4_23D")
        # self.up_subm_4 = spconv.SparseInverseConv3d(model_size*4, model_size*4, kernel_size=(2,2,2,1), indice_key='floor3', bias=False)
    ...
    def forward(self, sp_tensor):
        ...
        sp_tensor = self.STDB_1_1_1(sp_tensor)
        down_2, skip_1 = self.STDB_1_1_2(sp_tensor) #1/2

        down_2 = self.STDB_2_1_1(down_2)
        down_3, skip_2 = self.STDB_2_1_2(down_2) #1/4

        down_3 = self.STDB_3_1_1(down_3)
        down_4, skip_3 = self.STDB_3_1_2(down_3) #1/8

        down_4 = self.STDB_4_1_1(down_4)
        down_5, skip_4 = self.STDB_4_1_2(down_4) #1/16
