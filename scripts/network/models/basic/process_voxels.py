    ...
    def forward(self, features, coors):
        ...
        if self._with_cluster_center: 
            ...
            points_mean = self.map_voxel_center_to_point(
                coors, voxel_mean, mean_coors)
            # TODO: maybe also do cluster for reflectivity
            f_cluster = features[:, :3] - points_mean[:, :3] #距离中心的距离
            features_ls.append(f_cluster)
        ...
        for i, pfn in enumerate(self.pfn_layers):
            ...
            # # !debug
            # for submodule in pfn:
            #     if isinstance(submodule, nn.BatchNorm1d):
            #         print(f"Before forward pass:")
            #         print(f"  γ (weight): {submodule.weight.data}")
            #         print(f"  β (bias): {submodule.bias.data}")
            #         print(f"  running_mean: {submodule.running_mean}")
            #         print(f"  running_var: {submodule.running_var}")
            voxel_feats, voxel_coors = self.pfn_scatter(point_feats, coors)
            if i != len(self.pfn_layers) - 1:
                # need to concat voxel feats if it is not the last pfn
                feat_per_point = self.map_voxel_center_to_point(
