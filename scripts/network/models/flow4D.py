...
from .basic.embedder_model_flow4D import DynamicEmbedder_4D_ori
...
class Flow4D(nn.Module):
    def __init__(self, voxel_size = [0.2, 0.2, 0.2],
        ...
        self.diffusion_pc1 = CPDM(pseudo_image_dims=[grid_feature_size[0], grid_feature_size[1], grid_feature_size[2], num_frames], sample_step = 2)
    ...
    def forward(self, batch, training_flag):
        ...
        if training_flag:
            pcs_dict['pc1s_gt'] = batch["pc0"] + batch["flow"]
        for i in range(1, self.num_frames - 1):
        ...
        if training_flag:
            loss_pc1, restore_pc1 = self.diffusion_pc1.get_loss(voxel_feats_list_batch_dict, voxel_coors_list_batch_dict, pc0s_voxel_feature, pc01_tesnor_4d, batch_sizes)
            if random.random() < 0.5:
                pc01_tesnor_4d_restore = pc01_tesnor_4d.replace_feature(torch.cat((dict_4d['pc0s_voxel_feature'], restore_pc1), dim=0))
            else:
                pc01_tesnor_4d_restore = pc01_tesnor_4d
        else:
            restore_pc1 = self.diffusion_pc1.val_restore(voxel_feats_list_batch_dict, voxel_coors_list_batch_dict, pc0s_voxel_feature, pc01_tesnor_4d, batch_sizes)
            pc01_tesnor_4d_restore = pc01_tesnor_4d.replace_feature(torch.cat((dict_4d['pc0s_voxel_feature'], restore_pc1), dim=0))
