...
from .basic.embedder_model_flow4D import DynamicEmbedder_4D_ori, DynamicEmbedder_4D_offset_add_noise_NK, DynamicEmbedder_4D_restore, DynamicEmbedder_4D_pc0
...
class Flow4D(nn.Module):
    def __init__(self, voxel_size = [0.2, 0.2, 0.2],
        ...
        self.embedder_4D_offset_add_noise = DynamicEmbedder_4D_offset_add_noise_NK(voxel_size=voxel_size,
                                pseudo_image_dims=[grid_feature_size[0], grid_feature_size[1], grid_feature_size[2], num_frames], 
                                point_cloud_range=point_cloud_range,
                                feat_channels=point_output_ch)
    ...
    def forward(self, batch, training_flag):
        ...
        for change_frame in change_frames_list:
            ...
            for batch_id in range(batch_sizes):
                if change_frame == 'pc0':
                    ...
                    selected_pc0 = batch[change_frame][batch_id].reshape(-1,3) #! reshape N*K,3
        ...
        #! change_restore_pc0_all 转换恢复后点的坐标
        restore_point_pc0s = restore_point_dict_batch['pc0_all_ori']
        transform_restore_pc0s = [] #! init

        for batch_id in range(batch_sizes):
            selected_restore_pc0 = restore_point_pc0s[batch_id] 
            with torch.no_grad():
                if 'ego_motion' in batch:
                    pose_0to1 = batch['ego_motion'][batch_id] 
                else:
            ...
            transform_restore_pc0 = selected_restore_pc0 @ pose_0to1[:3, :3].T + pose_0to1[:3, 3] #t -> t+1 warping
            transform_restore_pc0s.append(transform_restore_pc0)
        
        restore_pc0_all_change = torch.stack(transform_restore_pc0s, dim=0) 


        restore_pc0_all = torch.cat((pc0s, restore_pc0_all_change), dim=1)
