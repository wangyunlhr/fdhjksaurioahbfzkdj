...
from .basic.embedder_model_flow4D import DynamicEmbedder_4D_less_to_more, DynamicEmbedder_4D_less
from .basic import cal_pose0to1

from .basic.network_4D import Network_4D, Seperate_to_3D, Point_head
from .basic.generator import CPDM
import os
import sys
...
class Flow4D(nn.Module):
    def __init__(self, voxel_size = [0.2, 0.2, 0.2],
        ...
        self.embedder_4D = DynamicEmbedder_4D_less_to_more(voxel_size=voxel_size,
        ...
        self.embedder_4D_less = DynamicEmbedder_4D_less(voxel_size=voxel_size,
                                        pseudo_image_dims=[grid_feature_size[0], grid_feature_size[1], grid_feature_size[2], num_frames], 
                                        point_cloud_range=point_cloud_range,
                                        feat_channels=point_output_ch)
        ...
        self.diffusion_pc1 = CPDM(pseudo_image_dims=[grid_feature_size[0], grid_feature_size[1], grid_feature_size[2], num_frames], sample_step = sample_step)
    ...
    def forward(self, batch, training_flag):
        ...
        if training_flag:
            dict_4d_more = self.embedder_4D(pcs_dict)
        dict_4d_less = self.embedder_4D_less(pcs_dict)
        pc01_tesnor_4d = dict_4d_less['4d_tensor']
        pc0_3dvoxel_infos_lst =dict_4d_less['pc0_3dvoxel_infos_lst']
        pc0_point_feats_lst =dict_4d_less['pc0_point_feats_lst']
        pc0_num_voxels = dict_4d_less['pc0_mum_voxels']

        voxel_feats_list_batch_dict = dict_4d_less['voxel_feats_list_batch_dict'] #字典batch特征
        voxel_coors_list_batch_dict = dict_4d_less['voxel_coors_list_batch_dict'] #字典 直接的indice
        pc0s_voxel_feature = dict_4d_less['pc0s_voxel_feature']

        if training_flag:
            loss_pc1, restore_pc0, restore_pc1 = self.diffusion_pc1.get_loss_multi(voxel_feats_list_batch_dict, voxel_coors_list_batch_dict, \
                                                                                   pc0s_voxel_feature, pc01_tesnor_4d, batch_sizes,\
                                                                                   dict_4d_more['voxel_feats_list_batch_dict'], dict_4d_more['voxel_coors_list_batch_dict'])
            if random.random() < 0.5:
                pc01_tesnor_4d_restore = pc01_tesnor_4d.replace_feature(torch.cat((restore_pc0, restore_pc1), dim=0))
            else:
                pc01_tesnor_4d_restore = pc01_tesnor_4d
        else:
            restore_pc0, restore_pc1 = self.diffusion_pc1.val_restore_multi(voxel_feats_list_batch_dict, voxel_coors_list_batch_dict, pc0s_voxel_feature, pc01_tesnor_4d, batch_sizes)
            pc01_tesnor_4d_restore = pc01_tesnor_4d.replace_feature(torch.cat((restore_pc0, restore_pc1), dim=0))
        ...
        pc_all_output_4d = self.network_4D(pc01_tesnor_4d_restore) #all = past, current, next 다 합친것
        self.timer[2].stop()
        ...
        if training_flag:
            model_res['restore_loss'] = loss_pc1
        return model_res