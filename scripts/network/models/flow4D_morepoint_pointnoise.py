...
from .basic.embedder_model_flow4D import DynamicEmbedder_4D_less_to_more_add_noise
from .basic import cal_pose0to1

from .basic.network_4D import Network_4D, Seperate_to_3D, Point_head, Network_3D
...
class Flow4D(nn.Module):
    def __init__(self, voxel_size = [0.2, 0.2, 0.2],
        ...
        self.embedder_4D = DynamicEmbedder_4D_less_to_more_add_noise(voxel_size=voxel_size,
    ...
    def forward(self, batch, training_flag):
        ...
        dict_4d = self.embedder_4D(pcs_dict, training_flag)
        pc01_tesnor_3d = dict_4d['3d_tensor']
        ...
        voxel_feats_list_batch_dict = dict_4d['voxel_feats_list_batch_dict'] #字典batch特征
        voxel_coors_list_batch_dict = dict_4d['voxel_coors_list_batch_dict'] #字典 直接的indice
        pc0s_voxel_feature = dict_4d['pc0s_voxel_feature']
        add_noise_dict = dict_4d['add_noise_dict']
        cond = self.network_3D(pc01_tesnor_3d)

        if training_flag:
            loss_pc1, restore_pc0, restore_pc1 = self.diffusion_pc1.get_loss_point_noise(voxel_feats_list_batch_dict, voxel_coors_list_batch_dict,\
                                                                                         pc0s_voxel_feature, pc01_tesnor_4d, batch_sizes, add_noise_dict)
        ...
        # else:
        #     restore_pc0, restore_pc1 = self.diffusion_pc1.val_restore_multi(voxel_feats_list_batch_dict, voxel_coors_list_batch_dict,\
