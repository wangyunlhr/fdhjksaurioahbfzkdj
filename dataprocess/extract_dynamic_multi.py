...
def compute_sceneflow(data_dir: Path, log_id: str, timestamps: Tuple[int, int], avm: ArgoverseStaticMap, stage: str, ts0: int) -> Dict[str, Union[np.ndarray, SE3]]:
    ...
    """
    def accumulate_cuboids(sweeps, cuboids, poses, stage): # include 5 frame
        if stage == 'start':
            target_idx = 0
        elif stage == 'end':
            target_idx = -2
        ...
        for time_idx in range(len(sweeps)):
            for id in cuboids[time_idx]:
                if id in cuboids[target_idx]:
                    ...
                    transformed_pts_list.append(transformed_pts)
        # classes_pts_all =np.concatenate(classes_pts_list, axis=0)   
        transformed_pts_all =np.concatenate(transformed_pts_list, axis=0)   

        return transformed_pts_all, target_idx #classes_pts_all, 


    def compute_flow(sweeps, cuboids, poses, target_idx, new_pc0):
    ...
    for i in range(len(sweeps)):
        pc = sweeps[i].xyz
        is_ground = avm.get_ground_points_boolean(poses[i].transform_point_cloud(pc))
        sweeps_no_ground.append(pc[~is_ground])


    transformed_pts_all, target_idx = accumulate_cuboids(sweeps_no_ground, cuboids, poses, stage) #! 累积点云
    assert ts0 == timestamps[target_idx]
    flow_0_1, classes_0, valid_0, ego_motion = compute_flow(sweeps, cuboids, poses, target_idx, transformed_pts_all)
...
def process_log(data_dir: Path, log_id: str, output_dir: Path, n: Optional[int] = None) :
    ...
    def create_group_data(group, pc, pose, flow_0to1=None, flow_valid=None, flow_category=None, ego_motion=None):
        if pc is not None:
    ...
    with h5py.File(output_dir/f'{log_id}.h5', 'a') as f:
        for cnt, ts0 in enumerate(timestamps):
            ...
            if cnt == len(timestamps) - 1:
                create_group_data(group, None, pose0.transform_matrix.astype(np.float32))
            else:
                if cnt < mid_frame:
                    stage = 'start'
                ...
                create_group_data(group, scene_flow['new_pc0'], pose0.transform_matrix.astype(np.float32), #! change to new_pc0
...
def main(
    ...
    output_dir: str ="/data1/dataset/av2/multi_box",
    av2_type: str = "sensor",
    data_mode: str = "val",
