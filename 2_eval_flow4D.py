...
def main(cfg):
    ...
    trainer.validate(model = mymodel, \
                     dataloaders = DataLoader(HDF5Dataset(cfg.dataset_path + f"/{cfg.av2_mode}", n_frames=cfg.num_frames, eval=True), batch_size=1, shuffle=False, num_workers=num_workers))
    wandb.finish()
