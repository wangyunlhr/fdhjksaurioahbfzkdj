...
class HDF5Dataset(Dataset):
    def __init__(self, directory, n_frames, eval = False):
        ...
        with open(os.path.join(self.directory, 'index_total.pkl'), 'rb') as f: #根据存储时候的以第四帧命名的group.key() timestamp   data_index.append([scene_id, timestamp])
            self.data_index = pickle.load(f)

        with open('/data0/code/Flow4D-main/conf/labeling.yaml', 'r') as file:
    ...
    def __getitem__(self, index_):
        if self.eval_index:
            ...
            scene_id, timestamp = self.data_index[index_] 
            # to make sure we have continuous frames
            # process data only save the valid frame
            # if self.scene_id_bounds[scene_id]["max_index"] == index_: 
            #     index_ = index_ - 1
            scene_id, timestamp = self.data_index[index_] 
        ...
        with h5py.File(os.path.join(self.directory, f'{scene_id}.h5'), 'r') as f: 
            for frame_i in range(5):
                res_dict[f'flows_5_frame_s_{frame_i}'] = torch.tensor(f[key][f'flows_5_frame_s_{frame_i}'][:])
                res_dict[f'flows_adjacent_s_{frame_i}'] = torch.tensor(f[key][f'flows_adjacent_s_{frame_i}'][:]) 
                res_dict[f'valid_5_frame_s_{frame_i}'] = torch.tensor(f[key][f'valid_5_frame_s_{frame_i}'][:]) 
                res_dict[f'valid_adjacent_s_{frame_i}'] = torch.tensor(f[key][f'valid_adjacent_s_{frame_i}'][:]) 
                res_dict[f'classes_s_{frame_i}'] = torch.tensor(f[key][f'classes_s_{frame_i}'][:]) 
                res_dict[f'point_s_{frame_i}'] = torch.tensor(f[key][f'point_s_{frame_i}'][:]) 
                res_dict[f'is_ground_0_s_{frame_i}'] = torch.tensor(f[key][f'is_ground_0_s_{frame_i}'][:]) 
                res_dict[f'pose_s_{frame_i}'] = torch.tensor(f[key][f'pose_s_{frame_i}'][:]) 
            ...
            if self.eval_index: 
                if self.mode == 'val':
                    ...
                    eval_mask = torch.ones(res_dict['point_s_3'].shape[0], 1, dtype=torch.bool) 
                    res_dict['eval_mask'] = eval_mask
                else:
                    raise ValueError(f"Invalid mode: {self.mode}. Only 'val' and 'test' are supported.")
